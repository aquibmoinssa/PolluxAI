...
### **Physical Parameters**: Outline the relevant physical parameters (e.g., mass, temperature, composition etc.).
### **Observables**: Specify the key observables required to study the science context.
### **Description of Desired Observations**: Detail the observational techniques, instruments, or approaches necessary to gather relevant data.
### **Science Impact**: Describe the science impact of the study based on the science objectives in one paragraph.
### **Overview: Generate an overview (maximum 3 paragraphs) based on the previous (Science Objectives, Physical Parameters, Observables, Description of Desired Observations, Science Impact) 
### **Observations Requirements Table**: Generate a table relevant to the Science Objectives, Physical Parameters, Observables and Description of Desired Observations with the following columns and at least 10 rows:
    ...
    - Flux Calibration: Based on the user input, provide a quantitative requirement for the absolute flux accuracy. State the value in Percentage (%). **Write a short justification (at least 250 words for all wavebands except simultaneous) for the value based on the user input in the same cell**.
    - Flux Stability: Based on the user input, provide a quantitative requirement for the relative photometric precision. State the value in Parts Per Million (ppm) over 1 hour. **Write a short justification (at least 250 words for all wavebands except simultaneous) for the value based on the user input in the same cell**.
    - Wavelength Calibration: Based on the user input, provide a quantitative requirement for the wavelength solution accuracy. State the value in Angstroms. **Write a short justification (at least 250 words for all wavebands except simultaneous) for the value based on the user input in the same cell**.
    - Wavelength Stability: Based on the user input, provide a quantitative requirement for the instrument's stability against wavelength drift. State the value as a Fraction of a pixel (decimal) over 1 hour. **Write a short justification (at least 250 words for all wavebands except simultaneous) for the value based on the user input in the same cell**.
...
- **Simultaneity: ** Mention in the Wavelength band column, if the science objectives can benefit from observations in more than one bands. Make sure to use just one row to specify the subset of wavebands for simultaneous observations.**
